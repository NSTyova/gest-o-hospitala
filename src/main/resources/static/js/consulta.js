...
$('#data').on('blur', function () {
    ...
    if (Date.parse(data)) {
        $.get('/consultas/horario/medico/'+ medico + '/data/' + data , function( result ) {
            $.each(result, function (k, v) {
                $("#horarios").append( 
    ...
    var table = $('#table-funcionario-historico').DataTable({
        ...
        columns : [
            ...
            {orderable : false,	data : 'id', "render" : function(id) {
                    ...
                    + id +'" role="button" data-toggle="modal" data-target="#confirm-modal1"><i class="fas fa-times-circle"></i></a>';
                }
            },
            {orderable : false,	data : 'id', "render" : function(id) {
